...
(assert (not (= def use)))
(assert (not (=> (and (and (>= pv10 0) (<= pv10 135299)) (forall ((?A_59 Int)) (=> (and (>= ?A_59 0) (<= ?A_59 (- pv10 1))) (= (sum 0 4 (select (select q ?A_59) tptp_sum_index)) 1.0)))) (forall ((?B_60 Int)) (let ((?v_2 (select x pv10)) (?v_1 (select sigma ?B_60)) (?v_4 (select sigma tptp_sum_index)) (?v_5 (sqrt (* 2.0 tptp_pi)))) (let ((?v_0 (- ?v_2 (select mu ?B_60))) (?v_3 (- ?v_2 (select mu tptp_sum_index)))) (=> (and (>= ?B_60 0) (<= ?B_60 (- 1))) (= (select (select q pv10) ?B_60) (divide (divide (* (exp (divide (divide (* ?v_0 ?v_0) tptp_minus_2) (* ?v_1 ?v_1))) (select rho ?B_60)) (* ?v_5 ?v_1)) (sum 0 4 (divide (* (exp (divide (divide (* ?v_3 ?v_3) tptp_minus_2) (* ?v_4 ?v_4))) (select rho tptp_sum_index)) (* ?v_5 ?v_4))))))))))))
(check-sat)
(exit)
