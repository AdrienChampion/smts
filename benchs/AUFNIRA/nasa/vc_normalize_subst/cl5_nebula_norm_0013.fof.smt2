...
(assert (not (= def use)))
(assert (let ((?v_0 (- 135300 1)) (?v_1 (- (select x pv83) (select mu pv35)))) (not (=> (and (and (and (= pv78 (sum 0 ?v_0 (select (select q pv79) pv35))) (= pv82 (sum 0 ?v_0 (* ?v_1 (* ?v_1 (select (select q pv83) pv35)))))) (>= pv35 0)) (<= pv35 (- 5 1))) true))))
(check-sat)
(exit)
