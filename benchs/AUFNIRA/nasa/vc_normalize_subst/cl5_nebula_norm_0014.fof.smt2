...
(assert (not (= def use)))
(assert (let ((?v_0 (- 135300 1))) (let ((?v_2 (= pv78 (sum 0 ?v_0 (select (select q pv79) pv35)))) (?v_4 (>= pv35 0)) (?v_5 (<= pv35 (- 5 1))) (?v_1 (- (select x pv83) (select mu pv35)))) (let ((?v_3 (* ?v_1 (* ?v_1 (select (select q pv83) pv35))))) (not (=> (and (and (and ?v_2 (= pv82 (sum 0 (- (+ 1 ?v_0) 1) ?v_3))) ?v_4) ?v_5) (and (and (and ?v_2 (= pv82 (sum 0 ?v_0 ?v_3))) ?v_4) ?v_5)))))))
(check-sat)
(exit)
