...
(assert (not (= def use)))
(assert (let ((?v_0 (- 135300 1))) (let ((?v_2 (= pv78 (sum 0 ?v_0 (select (select q pv79) pv35)))) (?v_3 (>= pv35 0)) (?v_4 (<= pv35 (- 5 1))) (?v_1 (* (select (select q pv81) pv35) (select x pv81)))) (not (=> (and (and (and (and (and ?v_2 (= pv80 (sum 0 (- pv39 1) ?v_1))) ?v_3) (>= pv39 0)) ?v_4) (<= pv39 ?v_0)) (and (and (and (= (+ (* (select (select q pv39) pv35) (select x pv39)) pv80) (sum 0 (- (+ 1 pv39) 1) ?v_1)) ?v_2) ?v_3) ?v_4))))))
(check-sat)
(exit)
