...
(assert (not (= def use)))
(assert (let ((?v_0 (- 135300 1))) (let ((?v_1 (= pv78 (sum 0 ?v_0 (select (select q pv79) pv35)))) (?v_3 (>= pv35 0)) (?v_4 (<= pv35 (- 5 1))) (?v_2 (* (select (select q pv81) pv35) (select x pv81)))) (not (=> (and (and (and ?v_1 (= pv80 (sum 0 (- (+ 1 ?v_0) 1) ?v_2))) ?v_3) ?v_4) (and (and (and ?v_1 (= pv80 (sum 0 ?v_0 ?v_2))) ?v_3) ?v_4))))))
(check-sat)
(exit)
