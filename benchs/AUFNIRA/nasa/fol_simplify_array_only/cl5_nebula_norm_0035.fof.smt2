...
(assert (not (= def use)))
(assert (let ((?v_1 (select (select q tptp_sum_index) pv35)) (?v_0 (- 135300 1))) (let ((?v_2 (= pv78 (sum 0 ?v_0 ?v_1)))) (not (=> (and (and (and ?v_2 (= pv80 (sum 0 (- (+ 1 ?v_0) 1) (* ?v_1 (select x tptp_sum_index))))) (>= pv35 0)) (<= pv35 (- 5 1))) ?v_2)))))
(check-sat)
(exit)
