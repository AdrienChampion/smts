...
(assert (not (= def use)))
(assert (not (=> (and (and (>= pv10 0) (<= pv10 (- 135300 1))) (forall ((?A_59 Int)) (=> (and (>= ?A_59 0) (<= ?A_59 (- pv10 1))) (= (sum 0 (- 5 1) (select (select q ?A_59) tptp_sum_index)) 1.0)))) (forall ((?B_60 Int)) (let ((?v_1 (select x pv10))) (let ((?v_0 (- (select (select center ?B_60) 0) ?v_1)) (?v_2 (- (select (select center tptp_sum_index) 0) ?v_1))) (=> (and (>= ?B_60 0) (<= ?B_60 (- 0 1))) (= (select (select q pv10) ?B_60) (divide (sqrt (* ?v_0 ?v_0)) (sum 0 (- 5 1) (sqrt (* ?v_2 ?v_2))))))))))))
(check-sat)
(exit)
