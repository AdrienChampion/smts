...
(assert (not (= def use)))
(assert (let ((?v_2 (select (select q tptp_sum_index) pv35)) (?v_0 (- 135300 1)) (?v_1 (- (select x tptp_sum_index) (select mu pv35)))) (not (=> (and (and (and (= pv78 (sum 0 ?v_0 ?v_2)) (= pv82 (sum 0 ?v_0 (* ?v_1 (* ?v_1 ?v_2))))) (>= pv35 0)) (<= pv35 (- 5 1))) true))))
(check-sat)
(exit)
