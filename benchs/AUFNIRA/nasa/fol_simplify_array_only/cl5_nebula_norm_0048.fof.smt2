...
(assert (not (= def use)))
(assert (let ((?v_1 (select (select q tptp_sum_index) pv35)) (?v_3 (select mu pv35)) (?v_2 (- 135300 1))) (let ((?v_0 (- (select x tptp_sum_index) ?v_3))) (let ((?v_5 (* ?v_0 (* ?v_0 ?v_1))) (?v_4 (- (select x pv41) ?v_3))) (not (=> (and (and (and (and (and (= pv78 (sum 0 ?v_2 ?v_1)) (= pv82 (sum 0 (- pv41 1) ?v_5))) (>= pv35 0)) (>= pv41 0)) (<= pv35 (- 5 1))) (<= pv41 ?v_2)) (= (+ (* ?v_4 (* ?v_4 (select (select q pv41) pv35))) pv82) (sum 0 (- (+ 1 pv41) 1) ?v_5))))))))
(check-sat)
(exit)
