...
(assert (not (= def use)))
(assert (let ((?v_0 (select (select q tptp_sum_index) pv35)) (?v_1 (- 135300 1))) (let ((?v_2 (* ?v_0 (select x tptp_sum_index)))) (not (=> (and (and (and (and (and (= pv78 (sum 0 ?v_1 ?v_0)) (= pv80 (sum 0 (- pv39 1) ?v_2))) (>= pv35 0)) (>= pv39 0)) (<= pv35 (- 5 1))) (<= pv39 ?v_1)) (= (+ (* (select (select q pv39) pv35) (select x pv39)) pv80) (sum 0 (- (+ 1 pv39) 1) ?v_2)))))))
(check-sat)
(exit)
