...
(assert (not (= def use)))
(assert (let ((?v_0 0.0) (?v_1 (select (select q tptp_sum_index) pv35))) (not (=> (and (and (= pv78 (sum 0 (- 135300 1) ?v_1)) (>= pv35 0)) (<= pv35 (- 5 1))) (=> (not (= ?v_0 pv78)) (= ?v_0 (sum 0 (- 0 1) (* ?v_1 (select x tptp_sum_index)))))))))
(check-sat)
(exit)
