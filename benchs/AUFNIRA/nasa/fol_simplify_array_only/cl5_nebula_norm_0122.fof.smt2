...
(assert (not (= def use)))
(assert (let ((?v_0 (<= pv10 (- 135300 1)))) (not (=> (and (and (and (>= pv10 0) ?v_0) (forall ((?A_59 Int)) (let ((?v_3 (select x pv10)) (?v_2 (- 5 1))) (let ((?v_1 (- (select (select center ?A_59) 0) ?v_3)) (?v_4 (- (select (select center tptp_sum_index) 0) ?v_3))) (=> (and (>= ?A_59 0) (<= ?A_59 (- (+ 1 ?v_2) 1))) (= (select (select q pv10) ?A_59) (divide (sqrt (* ?v_1 ?v_1)) (sum 0 ?v_2 (sqrt (* ?v_4 ?v_4)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 (- 5 1) (select (select q ?B_60) tptp_sum_index)) 1.0)))) ?v_0))))
(check-sat)
(exit)
