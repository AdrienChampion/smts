...
(assert (not (= def use)))
(assert (let ((?v_2 (<= pv10 (- 135300 1))) (?v_1 (- 5 1)) (?v_0 (- (select (select center tptp_sum_index) 0) (select x pv10)))) (not (=> (and (and (and (and (and (and (= pv70 (sum 0 (- (+ 1 ?v_1) 1) (sqrt (* ?v_0 ?v_0)))) (>= pv10 0)) (>= pv12 0)) ?v_2) (<= pv12 ?v_1)) (forall ((?A_59 Int)) (let ((?v_4 (select x pv10))) (let ((?v_3 (- (select (select center ?A_59) 0) ?v_4))) (=> (and (>= ?A_59 0) (<= ?A_59 (- pv12 1))) (= (select (select q pv10) ?A_59) (divide (sqrt (* ?v_3 ?v_3)) (sum 0 ?v_1 (sqrt (* ?v_0 ?v_0)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 ?v_1 (select (select q ?B_60) tptp_sum_index)) 1.0)))) ?v_2))))
(check-sat)
(exit)
