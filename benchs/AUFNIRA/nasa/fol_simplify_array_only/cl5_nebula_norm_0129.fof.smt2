...
(assert (not (= def use)))
(assert (let ((?v_0 (>= pv10 0))) (not (=> (and (and (and ?v_0 (<= pv10 (- 135300 1))) (forall ((?A_59 Int)) (let ((?v_4 (select x pv10)) (?v_2 (select sigma ?A_59)) (?v_6 (select sigma tptp_sum_index)) (?v_7 (sqrt (* 2.0 tptp_pi))) (?v_3 (- 5 1))) (let ((?v_1 (- ?v_4 (select mu ?A_59))) (?v_5 (- ?v_4 (select mu tptp_sum_index)))) (=> (and (>= ?A_59 0) (<= ?A_59 (- (+ 1 ?v_3) 1))) (= (select (select q pv10) ?A_59) (divide (divide (* (exp (divide (divide (* ?v_1 ?v_1) tptp_minus_2) (* ?v_2 ?v_2))) (select rho ?A_59)) (* ?v_7 ?v_2)) (sum 0 ?v_3 (divide (* (exp (divide (divide (* ?v_5 ?v_5) tptp_minus_2) (* ?v_6 ?v_6))) (select rho tptp_sum_index)) (* ?v_7 ?v_6)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 (- 5 1) (select (select q ?B_60) tptp_sum_index)) 1.0)))) ?v_0))))
(check-sat)
(exit)
