...
(assert (not (= def use)))
(assert (not (=> (and (and (and (and (and (>= pv10 0) (>= pv12 0)) (<= pv10 (- 135300 1))) (<= pv12 (- 5 1))) (forall ((?A_59 Int)) (let ((?v_1 (select x pv10))) (let ((?v_0 (- (select (select center ?A_59) 0) ?v_1)) (?v_2 (- (select (select center tptp_sum_index) 0) ?v_1))) (=> (and (>= ?A_59 0) (<= ?A_59 (- pv12 1))) (= (select (select q pv10) ?A_59) (divide (sqrt (* ?v_0 ?v_0)) (sum 0 (- 5 1) (sqrt (* ?v_2 ?v_2)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 (- 5 1) (select (select q ?B_60) tptp_sum_index)) 1.0)))) (forall ((?C_61 Int)) (let ((?v_4 (select x pv10))) (let ((?v_5 (- (select (select center tptp_sum_index) 0) ?v_4)) (?v_3 (- (select (select center ?C_61) 0) ?v_4))) (=> (and (>= ?C_61 0) (<= ?C_61 (- pv12 1))) (= (select (select q pv10) ?C_61) (divide (sqrt (* ?v_3 ?v_3)) (sum 0 (- 5 1) (sqrt (* ?v_5 ?v_5))))))))))))
(check-sat)
(exit)
