...
(assert (not (= def use)))
(assert (not (=> (and (and (and (>= pv10 0) (<= pv10 (- 135300 1))) (forall ((?A_59 Int)) (let ((?v_2 (select x pv10)) (?v_1 (- 5 1))) (let ((?v_0 (- (select (select center ?A_59) 0) ?v_2)) (?v_3 (- (select (select center tptp_sum_index) 0) ?v_2))) (=> (and (>= ?A_59 0) (<= ?A_59 (- (+ 1 ?v_1) 1))) (= (select (select q pv10) ?A_59) (divide (sqrt (* ?v_0 ?v_0)) (sum 0 ?v_1 (sqrt (* ?v_3 ?v_3)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 (- 5 1) (select (select q ?B_60) tptp_sum_index)) 1.0)))) (forall ((?C_61 Int)) (=> (and (>= ?C_61 0) (<= ?C_61 (- pv10 1))) (= (sum 0 (- 5 1) (select (select q ?C_61) tptp_sum_index)) 1.0))))))
(check-sat)
(exit)
