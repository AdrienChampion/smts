...
(assert (not (= def use)))
(assert (let ((?v_1 (select sigma tptp_sum_index)) (?v_0 (- (select x pv10) (select mu tptp_sum_index)))) (not (=> (and (and (and (and (and (>= pv10 0) (>= pv47 0)) (<= pv10 135299)) (<= pv47 4)) (forall ((?A_59 Int)) (let ((?v_4 (select x pv10)) (?v_3 (select sigma ?A_59)) (?v_5 (sqrt (* 2.0 tptp_pi)))) (let ((?v_2 (- ?v_4 (select mu ?A_59)))) (=> (and (>= ?A_59 0) (<= ?A_59 (- pv47 1))) (= (select (select q pv10) ?A_59) (divide (divide (* (exp (divide (divide (* ?v_2 ?v_2) tptp_minus_2) (* ?v_3 ?v_3))) (select rho ?A_59)) (* ?v_5 ?v_3)) (sum 0 4 (divide (* (exp (divide (divide (* ?v_0 ?v_0) tptp_minus_2) (* ?v_1 ?v_1))) (select rho tptp_sum_index)) (* ?v_5 ?v_1)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 4 (select (select q ?B_60) tptp_sum_index)) 1.0)))) (= 0.0 (sum 0 (- 1) (divide (* (exp (divide (divide (* ?v_0 ?v_0) tptp_minus_2) (* ?v_1 ?v_1))) (select rho tptp_sum_index)) (* (sqrt (* 2.0 tptp_pi)) ?v_1))))))))
(check-sat)
(exit)
