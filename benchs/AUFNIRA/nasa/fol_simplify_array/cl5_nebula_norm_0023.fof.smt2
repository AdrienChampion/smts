...
(assert (not (= def use)))
(assert (let ((?v_0 (- (select (select center tptp_sum_index) 0) (select x pv10)))) (not (=> (and (and (and (and (and (>= pv10 0) (>= pv12 0)) (<= pv10 135299)) (<= pv12 4)) (forall ((?A_59 Int)) (let ((?v_2 (select x pv10))) (let ((?v_1 (- (select (select center ?A_59) 0) ?v_2))) (=> (and (>= ?A_59 0) (<= ?A_59 (- pv12 1))) (= (select (select q pv10) ?A_59) (divide (sqrt (* ?v_1 ?v_1)) (sum 0 4 (sqrt (* ?v_0 ?v_0)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 4 (select (select q ?B_60) tptp_sum_index)) 1.0)))) (= 0.0 (sum 0 (- 1) (sqrt (* ?v_0 ?v_0))))))))
(check-sat)
(exit)
