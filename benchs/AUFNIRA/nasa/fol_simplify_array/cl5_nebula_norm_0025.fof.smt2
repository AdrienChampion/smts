...
(assert (not (= def use)))
(assert (not (=> (and (and (and (>= pv10 0) (<= pv10 135299)) (forall ((?A_59 Int)) (let ((?v_1 (select x pv10))) (let ((?v_0 (- (select (select center ?A_59) 0) ?v_1)) (?v_2 (- (select (select center tptp_sum_index) 0) ?v_1))) (=> (and (>= ?A_59 0) (<= ?A_59 4)) (= (select (select q pv10) ?A_59) (divide (sqrt (* ?v_0 ?v_0)) (sum 0 4 (sqrt (* ?v_2 ?v_2)))))))))) (forall ((?B_60 Int)) (=> (and (>= ?B_60 0) (<= ?B_60 (- pv10 1))) (= (sum 0 4 (select (select q ?B_60) tptp_sum_index)) 1.0)))) (= (sum 0 4 (select (select q pv10) tptp_sum_index)) 1.0))))
(check-sat)
(exit)
