...
object BasicTests extends Tester {
  ...
  def testParsers = {
    ...
    def test = {
      ...
      val smtLibParser = new SmtLibParsers[String, String, String] {
        ...
        lazy val smt2Expr: Parser[String] = {
          "(" ~ smt2Expr ~ ")" ^^ { case op~str~cp => op + str + cp } |
          """[^()]*""".r ~ smt2Expr^^ { case anyth ~ more => anyth + " " + more } | 
          """[^()]*""".r ^^ { case anyth => anyth }
        }
        lazy val smt2Ident: Parser[String] = """[a-zA-Z_][a-zA-Z0-9_$]*""".r ^^ { case ident => ident }
        lazy val smt2Num: Parser[String] = {
          """[1-9][0-9]*""".r ^^ { case ident => ident } |
          "0"
        }
        lazy val smt2Sort: Parser[String] = { "Bool" | "Int" | "Real" }
        def resultParse(s: String) =
          parse(resultParser,s) match {
      ...
      def testParsing(toParse: String) = {
        ...
        val result = smtLibParser.resultParse(toParse)
        verbln("  Result: " + result)
      }
